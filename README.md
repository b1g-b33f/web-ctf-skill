...
| `lfiquick.py` | Bounded GET path-traversal/LFI fast track: preserves a known-valid file URL and unrelated query fields, calibrates a same-directory miss, compares authenticated and anonymous reachability, and uses signature-backed Linux/Windows probes across a curated SecLists-derived core matrix. Reuses the exact winning style/depth for flag, environment, application-root, and configuration targets; an explicit `extended` profile adds encoded-dot/backslash, mixed, overlong, Unicode, and null-byte families. Scans headers and bodies, retains every response plus JSONL evidence, and treats weak differentials, throttling, gateways, and budget exhaustion as inconclusive |
